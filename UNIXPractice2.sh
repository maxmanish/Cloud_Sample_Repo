...
cut -d',' -f2 orders.csv
awk -F',' '{print $2}' orders.csv #bonus solution
...
awk -F',' '$4>5000' orders.csv #since pattern is given, printing full line is default behavior
...
awk -F',' '{print $1 , $2}' orders.csv # F is field separator. Default output field separator is space because of comma
...
sed '1d' orders.csv #delete line 1, prints remaining lines by default (sed prints every line by default unless told otherwise or default output is suppressed)
...
sed '/^$/d' orders.csv # ^$ → A line that has nothing in it (empty line), deleted. Prints remaining lines by default
